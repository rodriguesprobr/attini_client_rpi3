    ...
    if action == "start":
        try:
            ...
            while True:
                ...
                if send_status == 0:
                    gpio.blink(util.get_config("green_led_port"), 3, 0.125)
                elif send_status < 0:
                    gpio.light(util.get_config("green_led_port"), 0)
                    gpio.blink(util.get_config("red_led_port"), 20, 0.125)
                    gpio.light(util.get_config("green_led_port"),1)
                elif send_status == 1:
                    gpio.light(util.get_config("green_led_port"), 0)
                    gpio.blink(util.get_config("red_led_port"), 20, 0.125)
            ...
            gpio.light(util.get_config("green_led_port"), 0)
            gpio.light(util.get_config("blue_led_port"), 0)
            gpio.blink(util.get_config("red_led_port"), 30, 0.125)
            gpio.light(util.get_config("blue_led_port"),1)
            pass
        finally:
            gpio.cleanup()
